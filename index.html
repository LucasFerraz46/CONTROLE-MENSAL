...
<div class="wrap">
  ...
  <section class="panel">
    ...
    <table class="ledger">
      <thead>
        <tr><th>Data</th><th>Descrição</th><th style="text-align:right;">Valor</th><th style="text-align:right;">Saldo</th><th></th></tr>
      </thead>
    ...
    <form class="add-entry" id="addEntryForm">
      ...
      <div id="parcelasWrap">
        ...
        <input type="number" id="fParcelas" min="1" value="1">
      </div>
      ...
      <div class="parcelas-hint" id="parcelasHint">Em compras parceladas, informe o valor total da compra — cada parcela entra automaticamente no mês seguinte.</div>
    </form>
  </section>
...
(function(){
  ...
  function renderLedger(){
    ...
    if(month.entries.length===0){
      body.innerHTML = '<tr class="empty-row"><td colspan="5">nenhum lançamento neste mês</td></tr>';
    }else{
  ...
  document.getElementById('typeToggle').addEventListener('click', (e)=>{
    ...
    document.getElementById('parcelasWrap').style.display = btn.dataset.type==='despesa' ? 'block' : 'none';
    document.getElementById('parcelasHint').style.display = btn.dataset.type==='despesa' ? 'block' : 'none';
  });
  ...
  document.getElementById('addEntryForm').addEventListener('submit', (e)=>{
    ...
    const totalValor = parseValor(document.getElementById('fValor').value);
    if(isNaN(totalValor) || totalValor<=0){ alert('Informe um valor válido.'); return; }
    const dataStr = document.getElementById('fData').value;
    const descBase = document.getElementById('fDesc').value.trim() || '(sem descrição)';
    let parcelas = type==='despesa' ? parseInt(document.getElementById('fParcelas').value, 10) : 1;
    ...
    // valor por parcela, com o último ajustado para fechar a soma exata
    const parcelaValor = Math.round((totalValor/parcelas)*100)/100;
    const soma = parcelaValor*(parcelas-1);
    const ultimaParcelaValor = Math.round((totalValor - soma)*100)/100;
    ...
    for(let i=0; i<parcelas; i++){
      ...
      const valor = i===parcelas-1 ? ultimaParcelaValor : parcelaValor;
      const descricao = parcelas>1 ? `${descBase} (${i+1}/${parcelas})` : descBase;
      ensureMonth(parcelaMonthKey).entries.push({
        ...
        tipo: type,
        valor,
        grupoId: parcelas>1 ? groupId : null
      });
    }
    saveData();
    ...
    document.getElementById('fParcelas').value = 1;
    renderAll();
  });
